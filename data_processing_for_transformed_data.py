...
parser.add_argument("--dataset", default="java-med", type=str, help="data")
parser.add_argument("--part", default="testing", type=str, help="part")
...
def main():
    ...
    with WorkerExecutor(max_workers=worker) as executor:   
        for transformed_dataset in transformed_datasets:
            transformed_dataset_path = os.path.join(path, transformed_dataset, dataset, part)
            for subdir, dirs, files in os.walk(transformed_dataset_path):
                for project in dirs:
