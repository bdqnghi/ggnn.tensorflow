...
from utils.network.tbcnn import TBCNNModel
...
def form_model_path(opt):
    ...
    return "tbcnn_method_name_prediction" + "_" + "-".join(model_path)
...
def main(opt):
    ...
    print("Initializing tbcnn model...........")
    tbcnn = TBCNNModel(opt)
    # network.init_net_treecaps(30,30)
    print("Finished initializing tbcnn model...........")

    
    loss_node = tbcnn.loss
            ...
            for epoch in range(1,  opt.epochs + 1):
                ...
                for train_step, train_batch_data in enumerate(train_batch_iterator):
                    ...
                    _, err, scores = sess.run(
                            ...
                            feed_dict={
                                tbcnn.placeholders["node_types"]: train_batch_data["batch_node_types"],
                                tbcnn.placeholders["node_tokens"]:  train_batch_data["batch_nodes_tokens"],
                                tbcnn.placeholders["children_indices"]:  train_batch_data["batch_children_indices"],
                                tbcnn.placeholders["children_node_types"]: train_batch_data["batch_children_node_types"],
                                tbcnn.placeholders["children_node_tokens"]: train_batch_data["batch_children_node_tokens"],
                                tbcnn.placeholders["labels"]: train_batch_data["batch_labels"],
                                tbcnn.placeholders["is_training"]: True
                            }
                        )
