...
public void create() {
    ...
    for (int i = 0; i < args.length; i++) {
        ...
        if (isCubemap && i < 7)
            continue;
        if ("-etc1".equals(args[i]))
            isPackETC1 = true;
        if ("-etc1a".equals(args[i]))
            isAlphaAtlas = isPackETC1 = true;
        if ("-mipmaps".equals(args[i]))
            isGenMipMaps = true;
    }
    ...
    try {
        ...
        // depth (not supported)
        out.writeInt(0);
        // n array elements (not supported)
        out.writeInt(0);
        out.writeInt(nFaces);
        out.writeInt(nLevels);
        // No additional info (key/value pairs)
        out.writeInt(0);
        for (int level = 0; level < nLevels; level++) {
            int faceLodSize = images[0][level].getSize();
            int faceLodSizeRounded = (faceLodSize + 3) & ~3;
